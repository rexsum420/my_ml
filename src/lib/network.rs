...
impl Network<'_> {
    pub fn new<'a>(layers: Vec<usize>, activation: Activation<'a>, learning_rate: f64) -> Network<'a> {
        assert!(layers.len() >=  2, "At least two layers are required");
        let mut weights = vec![];
        let mut biases = vec![];
    ...
    pub fn forward(&mut self, inputs: Vec<f64>) -> Vec<f64> {
        if inputs.len() !=  self.layers[0] { panic!("Invalid number of inputs") }
    ...
    pub fn back_propogate(&mut self, outputs: Vec<f64>, targets: Vec<f64>) {
        if targets.len() != self.layers[self.layers.len() - 1]{ panic!("Incorrect number of targets")}
